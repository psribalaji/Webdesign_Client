...
function Menu() {
  ...
  useEffect(() => {
    api
      .get('http://localhost:8080/api/res/getMenu', user)
      .then((res) => {
  ...
  const handleRowUpdate = (newData, oldData, resolve) => {
    ...
    if (errorList.length < 1) {
      api
        .patch('http://localhost:8080/api/res/updateMenu', newData)
        .then((res) => {
  ...
  const handleRowDelete = (oldData, resolve) => {
    ...
    api
      .post('http://localhost:8080/api/res/deleteMenu', { _id: oldData._id })
      .then((res) => {
